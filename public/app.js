...
function handleMessage(msg) {
  switch (msg.type) {
    ...
    case 'sse_event':
      ...
      interceptTaskToolSSE(msg.event);
      break;
...
// ============================================================
// TASK PANEL (intercepts TaskCreate / TaskUpdate tool calls)
// ============================================================
...
// Task state: { id: { id, subject, description, status, activeForm, blockedBy:[], blocks:[] } }
state.tasks = {};
// Todo state: [ { id, content, status } ]
state.todos = [];
// Accumulate in-flight tool calls: { blockIndex: { name, inputJson } }
state.pendingTaskTools = {};
state.taskPanelCollapsed = false;

const TASK_STATUS_ICONS = {
  pending:     '\u25CB',  // ○
  in_progress: '\u25E6',  // ◦ (animated via CSS)
  completed:   '\u2713',  // ✓
  blocked:     '\u29B8',  // ⦸
  failed:      '\u2717',  // ✗
  cancelled:   '\u2014',  // —
...
// --- Drag support for task panel ---
{
  const header = taskPanel?.querySelector('.task-panel-header');
  let dragging = false, startX, startY, startLeft, startTop;

  header?.addEventListener('mousedown', (e) => {
    if (e.target.closest('.task-panel-close')) return; // don't drag on close btn
    dragging = true;
    ...
    // Switch from top/right to top/left positioning for drag
...
function interceptTaskToolSSE(event) {
  // Watch for TaskCreate / TaskUpdate tool calls in SSE stream
  if (event.eventType === 'content_block_start' && event.data?.content_block?.type === 'tool_use') {
    const cb = event.data.content_block;
...
function applyTaskUpdate(input) {
  ...
  if (input.addBlocks) {
    ...
    for (const dep of deps) {
      ...
      // Also add reverse dependency on the target
  ...
  // Auto-unblock: if this task completed, unblock downstream
  if (task.status === 'completed') {
    for (const downstreamId of task.blocks) {
      ...
      // If fully unblocked and was blocked, set to pending
...
function applyTodoWrite(input) {
  // TodoWrite replaces the entire todo list each time
...
function renderTaskPanel() {
  ...
  // Sort: in_progress first, then pending/blocked, then completed/failed/cancelled
