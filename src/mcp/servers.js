...
function listTools() {
  ...
  // Append compiled workflows as read-only tool entries
  const workflowsDir = path.join(path.dirname(path.dirname(__dirname)), 'capabilities', 'workflows');
  if (fs.existsSync(workflowsDir)) {
    for (const entry of fs.readdirSync(workflowsDir, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      const compiledPath = path.join(workflowsDir, entry.name, 'compiled.js');
      ...
      try {
        ...
        const toolName = (compiled.name || entry.name).replace(/-workflow$/, '').replace(/-/g, '_');
        // Skip if name conflicts with a meta.json tool
        ...
        const params = Object.entries(compiled.inputs || {}).map(([name, def]) => ({
          name,
          type: typeof def === 'string' ? 'string' : (def.type || 'string'),
        ...
        tools.push({
          ...
          builtin: true,
          source: 'workflow',
          workflow: compiled.name || entry.name,
          file: null,
          params,
        });
      } catch {}
